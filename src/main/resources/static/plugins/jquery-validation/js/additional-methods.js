...
$.validator.addMethod("ziprange", function(value, element) {
	...
	//自定义方法 start g20416 20170323
	//自定义url方法 B/S系统url验证
	jQuery.validator.addMethod( "bsurl", function( value, element ) {
	...
	//自定义名称方法 只能输入中文
	jQuery.validator.addMethod("zh_and_en", function(value, element) {
		return this.optional(element) || /^[\u4e00-\u9fa5]+$/i.test(value);
	}, "只能输入中文和字母");
	//自定义名称方法 只能输入中文和字母
	jQuery.validator.addMethod("zh_and_en2", function(value, element) {
		return this.optional(element) || /^[\u4e00-\u9fa5_a-zA-Z]+$/i.test(value);
	}, "只能输入中文和字母");
    //自定义名称方法 只能输入中文和字母“-”
    jQuery.validator.addMethod("authorityFunc", function(value, element) {
        return this.optional(element) || /^[-?\u4e00-\u9fa5_a-zA-Z]+$/i.test(value);
    }, "只能输入中文、英文与符号“-”的组合");
	//自定义名称方法 只能输入大写字母
	jQuery.validator.addMethod("upperenonly", function(value, element) {
		return this.optional(element) || /^[A-Z]+$/i.test(value);
	}, "只能输入大写字母");
	jQuery.validator.addMethod("upperenonly2", function(value, element) {
		return this.optional(element) || /^[_?A-Z]+$/i.test(value);
	}, "只能输入大写字母和下划线的组合");
	jQuery.validator.addMethod("upenandnum", function(value, element) {
		return this.optional(element) || /^[_?\-?A-Z0-9]+$/i.test(value);
	}, "只能输入大写字母和数字以及“_”或“-”组合");
    ...
    jQuery.validator.addMethod("isFax", function(value, element) {
        var phone = /^((\d2,3)|(\d{3}\-))?(0\d2,3|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/;
        return this.optional(element) || (phone.test(value));
    }, "请填写正确的传真");
    // 联系电话(手机/电话皆可)验证
    ...
    jQuery.validator.addMethod("isIdCode", function(value, element) {
        var length = value.length;
        var idCode = /^[1-9]{1}[0-9]{14}$|^[1-9]{1}[0-9]{16}([0-9]|[xX])$/;
        return this.optional(element) || idCode.test(value);
    }, "请输入18位身份证号");
    // 17.10.10新加的车牌号验证
    jQuery.validator.addMethod("isCarCode", function(value, element) {
        var length = value.length;
        var carCode = /^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z_0-9]{5}$/;
        return this.optional(element) || (length == 7 && carCode.test(value));
    }, "请填写正确的车牌号");
    // 17.10.10新加的车牌号验证
